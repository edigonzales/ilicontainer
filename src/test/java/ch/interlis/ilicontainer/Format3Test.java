...
import ch.interlis.ilicontainer.index.Keys;
...
public class Format3Test {
  @Rule public TemporaryFolder temp = new TemporaryFolder();

  @Test
  public void independentBinaryFixtureAndWriterAgree() throws Exception {
    byte[] fixture = Files.readAllBytes(Paths.get("src/test/resources/format3/binary-index.ilic"));
    Path output = temp.getRoot().toPath().resolve("fixture.ilic");
    try (ch.interlis.ilicontainer.index.ExternalSort sort =
            new ch.interlis.ilicontainer.index.ExternalSort(temp.getRoot().toPath(), 16384, true);
        RandomAccessFile out = new RandomAccessFile(output.toFile(), "rw")) {
      Frames.header(out);
      sort.add("F\0" + 0, new byte[] {1});
      sort.add("F\0" + 1, new byte[] {2});
      try (CloseableIterator<ch.interlis.ilicontainer.index.ExternalSort.Entry> entries =
          sort.finish()) {
        long root =
            ch.interlis.ilicontainer.index.BTree.build(out, entries, temp.getRoot().toPath());
        Frames.footer(out, root, 0);
      }
    }
    assertArrayEquals(fixture, Files.readAllBytes(output));
    ReadMetrics m = new ReadMetrics();
    try (ch.interlis.ilicontainer.remote.LocalSource source =
        new ch.interlis.ilicontainer.remote.LocalSource(output, m)) {
      long[] roots = Frames.footer(source);
      ch.interlis.ilicontainer.index.BTree tree =
          new ch.interlis.ilicontainer.index.BTree(
  ...
  public void legacyHeadersAreExplicitlyRejected() throws Exception {
    for (int version = 1; version <= 2; version++) {
      ByteBuffer bytes = ByteBuffer.allocate(16).putLong(Frames.MAGIC).putInt(version).putInt(0);
      try {
  ...
  private Path create(String encoding, boolean order) throws Exception {
    ...
    Path target = temp.getRoot().toPath().resolve(encoding + order + ".ilic");
  ...
  public void mergedCorruptionNeverCachesAnUnverifiedBatch() throws Exception {
    Path path = temp.getRoot().toPath().resolve("corrupt-batch.ilic");
    ...
    try (RangeServer server = new RangeServer(path)) {
      ...
      try (ch.interlis.ilicontainer.remote.HttpRangeSource source =
          new ch.interlis.ilicontainer.remote.HttpRangeSource(
  ...
  public void spatialOrderingAndMultiPageStrAgreeWithIndependentPointScan() throws Exception {
    ...
    for (String encoding : Arrays.asList("iom", "wkb")) {
      ...
      w.chunkSize = 4096;
      Path file = temp.getRoot().toPath().resolve("grid-" + encoding + ".ilic");
