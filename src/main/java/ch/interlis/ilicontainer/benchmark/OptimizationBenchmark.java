...
public final class OptimizationBenchmark {
  ...
  private OptimizationBenchmark(JsonNode config) {
    ...
    container = output.resolve("data.ilic");
  ...
  private void save() throws Exception {
    ...
    Path jar =
        Paths.get(IliContainer.class.getProtectionDomain().getCodeSource().getLocation().toURI());
    ...
    try {
      ...
      if (!directoryOnly) {
        if (reference)
          ...
          Class<?> type = Class.forName("ch.interlis.ilicontainer.api.SpatialIndexOptions");
      ...
      if (!reference)
        try (IliContainer c = IliContainer.open(container)) {
          Object stats =
              Class.forName("ch.interlis.ilicontainer.container.StorageDiagnostics")
                  .getMethod("inspect", IliContainer.class)
                  .invoke(null, c);
      ...
      for (int repeat = 0; repeat < config.path("repeats").asInt(3); repeat++) {
        ...
        start = System.nanoTime();
        long count = 0;
        IoxReader reader = IliContainer.stream(Files.newInputStream(container));
        try {
          IoxEvent e;
            ...
            for (String query : queries) {
              ...
              try (IliContainer c =
                  delay < 0
                      ...
                      : IliContainer.open(server.uri(), remote)) {
  ...
  private IliContainer openLocal(Path path, RemoteOptions options) throws Exception {
    if (reference) return IliContainer.open(path);
    return (IliContainer)
        IliContainer.class
  ...
  private long query(IliContainer c, String query, boolean gis) throws Exception {
    if (query.equals("tid"))
    ...
    if (query.equals("fid") && !gis) {
      try (Fragment f =
              (Fragment) IliContainer.class.getMethod("getFid", long.class).invoke(c, fid);
          Stream<SelectedObject> stream = f.objects()) {
        return stream.count();
      }
    }
