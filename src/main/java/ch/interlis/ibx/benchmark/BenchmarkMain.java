...
@Command(name = "ibx-benchmark", mixinStandardHelpOptions = true)
public final class BenchmarkMain implements Callable<Integer> {
      ...
      for (String encoding : geometryMatrix ? new String[] {"iom", "wkb"} : new String[] {"iom"})
        for (String codec : codecs)
          for (int size : sizes) {
            ...
            Path core = output.resolve(variant + ".ibx");
            ...
            try (IbxContainer c = IbxContainer.open(core);
            ...
            for (int r = 0; r < repeats; r++) {
              try (InputStream in = Files.newInputStream(core)) {
                Map<String, Object> row = scan(IbxContainer.stream(in));
            ...
            Path spatial = output.resolve(variant + "-spatial.ibx");
  ...
  private Integer refreshSpatialResults() throws Exception {
    ...
    for (String variant : variants) {
      Path core = output.resolve(variant + ".ibx"),
          spatial = output.resolve(variant + "-spatial.ibx");
      if (modelMetadata == null) {
        try (IbxContainer container = IbxContainer.open(core)) {
  ...
  private void gisAccess(Path file, String variant, boolean spatial) throws Exception {
    try (RangeServer server = new RangeServer(file)) {
      for (boolean remote : new boolean[] {false, true})
        for (int r = 0; r < repeats; r++) {
          try (IbxContainer c = remote ? IbxContainer.open(server.uri()) : IbxContainer.open(file);
      ...
      for (boolean remote : new boolean[] {false, true})
        for (int r = 0; r < repeats; r++)
          for (String query :
            ...
            try (IbxContainer c =
                remote ? IbxContainer.open(server.uri()) : IbxContainer.open(file)) {
              for (String cache : Arrays.asList("cold", "warm")) {
                if (cache.equals("cold")) c.clearCache();
                c.metrics().reset();
                Meter meter = new Meter();
  ...
  private Fragment select(IbxContainer c, String query) throws Exception {
